...
def compute_sql_complexity(sql):
    ...
    number of JOINs, subqueries, CTEs, GROUP BY,
    HAVING, ORDER BY, UNION, etc.
    """
    ...
    # Nested SELECTs
    score += max(0, sql_upper.count("SELECT") - 1)
    ...
    score += sql_upper.count("INTERSECT")

    score += sql_upper.count("EXCEPT")

    score += sql_upper.count("UNION")
...
def get_llm_confidence(agent, question, schema_context):
    ...
    prompt = f"""
You are an expert SQL agent. Given a database schema and question,
...
def load_evaluatable_questions():
    ...
    gold_sql_files = [
        f
        for f in os.listdir(GOLD_SQL_DIR)
    ...
    for sql_file in gold_sql_files:
        ...
        with open(
            os.path.join(GOLD_SQL_DIR, sql_file),
            "r"
        ) as f:
...
def collect_signals():
    ...
    few_shot = get_relevant_few_shot_examples(
        num_examples=3
    )
    ...
    print(
        f"Collecting signals for {len(data)} questions...\n"
    )
    ...
    for item in tqdm(
        ...
        if db_id not in schema_cache:
            ...
            try:
                ...
                schema_cache[db_id] = SchemaRetriever(
                    db_path
                )
        ...
        cos_sim_gap = (
            cos_sim_top1 - cos_sim_top2
        )

        cos_sim_variance = float(
            np.var(similarities)
        )

        num_tables = len(
            retriever.table_names
        )
        ...
        # -------------------------------------------------
        # Round 1:
        # Top-3 schema context
        # -------------------------------------------------

        schema_context_r1, ranked_r1 = (
            retriever.get_scoped_context(
                question,
                top_k=3
            )
        )
        ...
        sql_complexity = compute_sql_complexity(
            sql_r1
        )
        ...
        if sql_r1:
            ...
            success_r1, result_r1 = (
                agent.execute_query(sql_r1)
            )
        ...
        if sql_r2:
            ...
            success_r2, result_r2 = (
                agent.execute_query(sql_r2)
            )
        ...
        print(
            ...
            f"cos={cos_sim_top1:.3f}, "
            f"complexity={sql_complexity}, "
            f"r1={'✓' if r1_correct else '✗'}, "
            f"r2={'✓' if r2_correct else '✗'}, "
    ...
    print(
        "EGSR helped: "
        f"{sum(1 for r in signal_data if r['egsr_helped'])}"
    )

    print(
        "EGSR did NOT help: "
        f"{sum(1 for r in signal_data if not r['egsr_helped'])}"
    )
